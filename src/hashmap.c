 ...
 */
static unsigned int hashmap_hash_int(struct hashmap *m, const char* keystring){
 ...
 */
static int hashmap_hash(struct hashmap *m, const char* key) {
	...
	for(int i = 0; i < MAX_CHAIN_LENGTH; i++){
		...
		if(elem->in_use == 0) return curr;
		if(elem->in_use == 1 && (strcmp(elem->key, key) == 0)) return curr;
 ...
 */
int hashmap_put(struct hashmap *m, const char* key, void *value) {
	/* Find a place to put our value */
	int index = hashmap_hash(m, key);
	while (index == MAP_FULL) {
		if (hashmap_rehash(m) == MAP_OMEM) return MAP_OMEM;
 ...
 */
int hashmap_get(struct hashmap *m, const char* key, void **arg) {
	/* Find data location */
	int curr = hashmap_hash_int(m, key);
 ...
 */
int hashmap_iterate(struct hashmap *m, hashmap_iter_cb f, void *cl) {
	...
	for(int i = 0; i < m->table_size; i++) {
		struct element *elem =
			m->data + (sizeof(struct element) + m->itemsize) * i;
		if (elem->in_use != 0) {
			void *item = (void*)elem->data;
			int status = f(cl, item);
			if (status != MAP_OK) {
				return status;
			}
		}
